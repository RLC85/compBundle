<?php

namespace CIS\Bundle\CompentencyBundle\Entity;

use Doctrine\ORM\Mapping as ORM;

/**
 * Tag
 */
class Tag
{
    /**
     * @var integer
     */
    private $id;

    /**
     * Constructor
     */
    public function __construct()
    {
        $this->compentencies = new \Doctrine\Common\Collections\ArrayCollection();
    }

    /**
     * Get id
     *
     * @return integer 
     */
    public function getId()
    {
        return $this->id;
    }
    /**
     * @var string
     */
    private $name;

    /**
     * @var string
     */
    private $slug;

    /**
     * @var \CIS\Bundle\CompentencyBundle\Entity\CompentencyTag
     */
    private $compentencyTags;

    /**
     * @var \Doctrine\Common\Collections\Collection
     */
    private $compentencies;


    /**
     * Set name
     *
     * @param string $name
     * @return Tag
     */
    public function setName($name)
    {
        $this->name = $name;

        return $this;
    }

    /**
     * Get name
     *
     * @return string 
     */
    public function getName()
    {
        return $this->name;
    }

    /**
     * Set slug
     *
     * @param string $slug
     * @return Tag
     */
    public function setSlug($slug)
    {
        $this->slug = $slug;

        return $this;
    }

    /**
     * Get slug
     *
     * @return string 
     */
    public function getSlug()
    {
        return $this->slug;
    }

    /**
     * Set compentencyTags
     *
     * @param \CIS\Bundle\CompentencyBundle\Entity\CompentencyTag $compentencyTags
     * @return Tag
     */
    public function setCompentencyTags(\CIS\Bundle\CompentencyBundle\Entity\CompentencyTag $compentencyTags = null)
    {
        $this->compentencyTags = $compentencyTags;

        return $this;
    }

    /**
     * Get compentencyTags
     *
     * @return \CIS\Bundle\CompentencyBundle\Entity\CompentencyTag 
     */
    public function getCompentencyTags()
    {
        return $this->compentencyTags;
    }

    /**
     * Add compentencies
     *
     * @param \CIS\Bundle\CompentencyBundle\Entity\Compentency $compentencies
     * @return Tag
     */
    public function addCompentency(\CIS\Bundle\CompentencyBundle\Entity\Compentency $compentencies)
    {
        $this->compentencies[] = $compentencies;

        return $this;
    }

    /**
     * Remove compentencies
     *
     * @param \CIS\Bundle\CompentencyBundle\Entity\Compentency $compentencies
     */
    public function removeCompentency(\CIS\Bundle\CompentencyBundle\Entity\Compentency $compentencies)
    {
        $this->compentencies->removeElement($compentencies);
    }

    /**
     * Get compentencies
     *
     * @return \Doctrine\Common\Collections\Collection 
     */
    public function getCompentencies()
    {
        return $this->compentencies;
    }
}
